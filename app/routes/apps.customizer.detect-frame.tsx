  ...
  try {
    ...
    if (cached && cached.frameDetectedAt) {
      const hoursSinceCheck =
        (Date.now() - cached.frameDetectedAt.getTime()) / (1000 * 60 * 60);
    ...
    // Threshold is 15%: the largest connected non-opaque region must cover at
    // least 15% of the image to be considered a frame. This is low enough to
    // catch semi-transparent frame borders (e.g. Magnet ~18%) while high enough
    // to reject product images with only minor transparency (noise/edges).
    const results = await findTransparentImages(imageUrls, 15);
