...
function App() {
  ...
  const initialCustomFrameUrl = rootElement?.dataset?.customFrameUrl || ''
  const initialHasCustomFrame = rootElement?.dataset?.hasCustomFrame === 'true'
  ...
  const [customFrameUrl, setCustomFrameUrl] = useState(initialCustomFrameUrl)
  const [hasCustomFrame, setHasCustomFrame] = useState(initialHasCustomFrame)
  ...
  // Generate auto frame from background image (always generate if product image exists)
  ...
  useEffect(() => {
    ...
    if (productImageUrl) {
      ...
      if (frameCache.current[productImageUrl]) {
        console.log('Using cached auto-generated frame for:', productImageUrl)
        setAutoGeneratedFrameUrl(frameCache.current[productImageUrl])
        setIsFrameLoading(false)
        return
      }
      
      console.log('DEBUG: Generating auto frame from product image:', productImageUrl)
      ...
      fetch(window.location.origin + '/apps/customizer/generate-frame?t=' + timestamp, {
        ...
        .then(data => {
          if (data.frameUrl) {
            console.log('Auto frame generated successfully')
            // Cache the frame URL
            frameCache.current[productImageUrl] = data.frameUrl
            setAutoGeneratedFrameUrl(data.frameUrl)
          } else {
            console.error('Auto frame generation failed:', data.error)
            setAutoGeneratedFrameUrl(null)
          }
          setIsFrameLoading(false)
        })
        .catch(err => {
          console.error('Auto frame generation error:', err)
          setAutoGeneratedFrameUrl(null)
          setIsFrameLoading(false)
        })
    } else {
      // No product image - no auto frame
      setAutoGeneratedFrameUrl(null)
      setIsFrameLoading(false)
    }
  }, [productImageUrl])
  ...
  useEffect(() => {
    const preloadImages = () => {
      ...
      if (customFrameUrl) {
        const customFrameImg = new Image()
        customFrameImg.src = customFrameUrl
      }
    ...
    preloadImages()
  }, [autoGeneratedFrameUrl, customFrameUrl, phoneCaseUrl])
  ...
  const handleDownload = async (format) => {
    ...
    try {
      ...
      if (hasCustomFrame && customFrameUrl) {
        try {
          const customFrameImg = document.querySelector('.phone-case-frame.custom-frame img')
          if (customFrameImg) {
            console.log('🖼️ Loading custom frame image')
            await new Promise((resolve, reject) => {
              const timeout = setTimeout(() => reject(new Error('Custom frame timeout')), 10000)
              const frameImg = new Image()
              frameImg.crossOrigin = 'anonymous'
              frameImg.onload = () => {
                clearTimeout(timeout)
                console.log('⏱️ Custom frame loaded')
              ...
              frameImg.src = customFrameImg.src
            })
          }
        } catch (error) {
          console.warn('Custom frame not loaded, continuing:', error.message)
        }
      }
    ...
    try {
      ...
      if (hasCustomFrame && customFrameUrl) {
        try {
          ...
          console.warn('Custom frame not loaded for complete image:', error.message)
        }
      }
      ...
      if (hasCustomFrame && customFrameUrl) {
        try {
          ...
          console.warn('Custom frame not loaded for empty case:', error.message)
        }
      }
    ...
    try {
      ...
      if (hasCustomFrame && customFrameUrl) {
        try {
          ...
          console.warn('Custom frame not loaded for print:', error.message)
        }
      }
    ...
    try {
      ...
      if (hasCustomFrame && customFrameUrl) {
        try {
          ...
          console.warn('Custom frame not loaded for preview:', error.message)
        }
      }
      ...
      <div className="main-container">
        ...
        <div className="phone-section" onClick={handlePhoneScreenClick}>
          <div className="phone-case">
            ...
            {/* Custom Frame Layer - Show if custom frame exists */}
            {hasCustomFrame && customFrameUrl && !isFrameLoading && (
