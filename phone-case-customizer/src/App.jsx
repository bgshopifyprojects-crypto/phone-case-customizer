...
function App() {
  ...
  useEffect(() => {
    async function loadFrame() {
      ...
      if (frameCache.current[productImageUrl]) {
        console.log('⚡ PYTHON FRAME SYSTEM: Using cached frame for:', productImageUrl)
      ...
      if (frameLoadingRef.current[productImageUrl]) {
        console.log('⏳ PYTHON FRAME SYSTEM: Already loading frame for this image, skipping duplicate request')
        return
      }
      
      console.log('🐍 PYTHON FRAME SYSTEM: Starting frame generation for:', productImageUrl)
      ...
      try {
        ...
        const rootElement = document.getElementById('phone-case-root')
        const productImageUrlsStr = rootElement?.dataset?.productImageUrls || ''
        const productImageUrls = productImageUrlsStr ? productImageUrlsStr.split(',').map(url => url.trim()) : [productImageUrl]
        
        // Step 1: Detect transparent frame image
        console.log('🐍 PYTHON: Step 1 - Detecting transparent frame with OpenCV...')
        const detectResponse = await fetch(window.location.origin + '/apps/customizer/detect-frame', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            productId: currentVariantId || 'unknown',
            imageUrls: productImageUrls.map(url => url.startsWith('http') ? url : 'https:' + url),
        ...
        console.log('🐍 PYTHON: Step 2 - Generating frame layer with Python/OpenCV...')
        const generateResponse = await fetch(window.location.origin + '/apps/customizer/generate-frame-python', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            productId: currentVariantId || 'unknown',
            referenceFrameUrl: detectData.frameImageUrl || null,
            targetImageUrl: productImageUrl.startsWith('http') ? productImageUrl : 'https:' + productImageUrl,
            tolerance: 30
          })
        })
        ...
        if (generateData.success && generateData.frameUrl) {
          ...
          console.log('⚠️ PYTHON FRAME SYSTEM: No frame available - customizer will work without frame layer')
          setAutoGeneratedFrameUrl(null)
        }
      } catch (err) {
        console.error('❌ PYTHON FRAME SYSTEM: Error during frame generation:', err)
  ...
  useEffect(() => {
    console.log('📊 Modal state changed - showDownloadModal:', showDownloadModal)
  }, [showDownloadModal])
  ...
  const handleDownload = async (format) => {
    ...
    try {
      ...
      if (autoGeneratedFrameUrl) {
        try {
          const autoFrameImg = document.querySelector('.phone-case-frame.auto-frame img')
          if (autoFrameImg) {
            console.log('🖼️ Loading auto-generated frame image')
            await new Promise((resolve, reject) => {
              const timeout = setTimeout(() => reject(new Error('Auto frame timeout')), 10000)
...
<svg width="600" height="1000" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    try {
      ...
      if (autoGeneratedFrameUrl) {
        try {
          ...
          console.warn('Auto frame not loaded for complete image:', error.message)
        }
      }
  ...
  return (
    <div className="phone-case-modal">
      ...
      {/* Download Modal */}
      {showDownloadModal && (
        <div className="modal-overlay" onClick={() => { console.log('Modal overlay clicked'); setShowDownloadModal(false); }}>
          <div className="modal-content" onClick={(e) => { console.log('Modal content clicked'); e.stopPropagation(); }}>
            <div className="modal-header">
              <h2>Tasarımı İndir</h2>
              <button className="modal-close" onClick={(e) => { console.log('Close button clicked'); e.stopPropagation(); setShowDownloadModal(false); }}>×</button>
            </div>
      ...
      {/* Preview Modal */}
      {showPreviewModal && (
        <div className="preview-modal-overlay" onClick={() => { console.log('Preview overlay clicked'); setShowPreviewModal(false); }}>
          <button className="preview-modal-close" onClick={(e) => { console.log('Preview close button clicked'); e.stopPropagation(); setShowPreviewModal(false); }}>×</button>
